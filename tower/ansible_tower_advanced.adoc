...
One very common question relates to how to structure their Git repositories. The first recommendation is to separate _specific code_ from _reusable code_ from _data_:

specific code:: playbooks and their direct dependencies you're not sharing or not planning to share outside the realm of your project or team. We'll see how to structure those after.
generic code:: those are basically Ansible roles to use across multiple projects. Given the current possibilities of `ansible-galaxy` to pull those roles, you will need one Git repository for each role (this might change with https://galaxy.ansible.com/docs/mazer/index.html[Mazer]). How roles are structured is https://galaxy.ansible.com/docs/contributing/creating_role.html[sufficiently described] and should be known. We'll discuss here how to use such role repositories within Tower.
