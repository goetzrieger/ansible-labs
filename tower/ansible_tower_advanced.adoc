...
Create credentials with this parameters:
...
* Hints: The credential type name is *Source Control*, as *inputs* you need *user* and *password*.
...
In one of the Tower web UI go to *Resources->Credentials*. Now:
...
Now with the SCM credentials configured, either in one of the web UI or using *tower-cli* create a *Project* for the *apache* Gitea repository. The parameters are:

* *Name:* Apache
* *SCM TYPE:* Git
* *SCM URL:* \http://control.example.com/gitea/git/apache.git
* *SCM Credentials:* Gitea Credentials
* *Organization:* Default
* Set all SCM update options to *true*  
* Make *tower-cli* wait until finished

==== Using *tower-cli* on the control host
...
+++ <details><summary> +++
*>> _Click here if you really want to see the web ui stuff_ <<*
+++ </summary><div> +++
...
So far nothing special regarding the Ansible Tower objects you created. But we are working in a clustered environment. Login to the other Tower instances Web UIs (the ones you didn't configured the inventory and credentials on). 
...
Now you are ready to start a job in your Tower cluster. In one of the web UI's *Templates* view select the new Job Template and run it by clicking the rocket icon. Again this is at first not different from a standard Tower.

TIP: But as this is a cluster of active Tower instances every instance could have run the job. And the Job output in Tower's web UI doesn't tell you where it run, just the instance group.
...
Compliance requirements are often grouped into independent categories. The tasks of can often be executed in parallel because they do not conflict with each other. 
...
The main reommendation is to put content in a version control system like Git or SVN. This comes naturally since Ansible code is usually in text form anyway, and thus can be managed easily. The other main recommendation is to group your code by logical units, called "link:https://docs.ansible.com/ansible/latest/user_guide/playbooks_reuse_roles.html[roles]" in Ansible: have all code, config templates and files for the apache web server in one role, and all code, configuration and sql statements for the database in another role. That way the code becomes much better to read and handle, and roles can be made re-usable and shared between projects, teams or with the global community.
...
This command creates two inventory files: in this case we have different stages and keep them separate with dedicated files. Note that those files are right now still empty and need to be filled with content to work properly.

In the current setup we have two instances. Let's assume that the `host1` instances is part of the staging environment, and that the instance `host2` is part of the production environment. To reflect that in the inventory files, they have to look like the following:
...
Next we add some more often needed default directories: first we add directories for host and group variables, and second we add a roles directory where the main part of our automation logic will be in. Most of the automation tasks should be organized in roles where it is possible and feasible.
...
Top better highlight how such a structure would look like we are going to add two roles. First we will create empty roles to fill in content later. Empty but already complete roles structures can easily be created with the command `ansible-galaxy`: it creates role skelletons with all appropriate files, directories and so on already in place.
...
IMPORTANT: Even if a good role is generally self-explanatory, the context and which one to use in which occasion generally isn't! That it makes sense to have a proper documentation. In terms of roles, the right way to document what it is about and what it is doing is the file ?meta/main.yml`.

The roles are empty, so we need to add a few tasks to each. In the last chapters we set up an Apache system and used some security tasks to showcase some Ansible functions. Let's add that code to our roles:
...
----
 ...
 We also need to create a playbook to call the roles from. This is often call `site.yml`, since it keeps the main code for the setup of our environment.
...
Since we so far created the code only locally on the control host, we need to add it to the repository and push it. First 
Add the code to the repository and push it. Check in the GUI if the code is actually there.

----
[ansible@control structured-content]$ git status
# On branch master
#
# Initial commit
#
...
[master (root-commit) d51f095] Adding inventories and apache security roles
 19 files changed, 285 insertions(+)
 create mode 100644 production
 create mode 100644 roles/apache/README.md
 create mode 100644 roles/apache/defaults/main.yml
 create mode 100644 roles/apache/handlers/main.yml
 create mode 100644 roles/apache/meta/main.yml
 create mode 100644 roles/apache/tasks/main.yml
 create mode 100644 roles/apache/tests/inventory
 create mode 100644 roles/apache/tests/test.yml
 create mode 100644 roles/apache/vars/main.yml
 create mode 100644 roles/security/README.md
 create mode 100644 roles/security/defaults/main.yml
 create mode 100644 roles/security/handlers/main.yml
 create mode 100644 roles/security/meta/main.yml
 create mode 100644 roles/security/tasks/main.yml
 create mode 100644 roles/security/tests/inventory
 create mode 100644 roles/security/tests/test.yml
 create mode 100644 roles/security/vars/main.yml
...
Writing objects: 100% (32/32), 4.03 KiB | 0 bytes/s, done.
Total 32 (delta 1), reused 0 (delta 0)
remote: LFS server support needs at least Git v2.1.2, disabled
remote: LFS server support needs at least Git v2.1.2, disabled
remote: LFS server support needs at least Git v2.1.2, disabled
To git@control.example.com:/git/structured-content.git
...
The code can now be launched. We start at the command line: call the playbook `site.yml` with the appropriate inventory and also requesting privilege escalation: `ansible-playbook -i staging site.yml -b`. Watch how the changes are done to the target machines. Afterwards, execute it against the production stage: `ansible-playbook -i production site.yml -b`.

==== From Tower

The new repository needs to be added as project. Feel free to use the web UI or use *tower-cli* like shown below.
...
Since we have a staged inventory now embedded in the repository, how about we re-use that in Tower? It is possible to add inventory files provided in a repository as a source to an inventory!
...
* In the now extended view, click on *Choose an inventory file*
* In the drop down menu, pick *production*
* Click the green *SAVE* button
* Down below in the view, click the image:green_plus.png[20,20] button again
* In the enw view, add as *NAME:* Staging
* *SOURCE:* Pick *Sourced from a Project*
* In the now extended view, click on *Choose an inventory file*
* In the drop down menu, pick *staging*
* Click the green *SAVE* button
...
To make sure that the project based inventory works, click on the *HOSTS* button and see that two hosts are listed there.

Now we will add ad template to execute the `site.yml` against both stages at the same time.
...
Since adding anoter template is something you are quite familiar with by now, let's just use `tower-cli`:
...
Launch the playbook from within Tower and watch the results.

=== Adding external roles

So far we only structured content inside one repository. While this drastically reduces complexity already, the largest benefit is in sharing roles among multiple teams or departments and keep them in a central place. In this section we will show how to reference such shared roles in your code and execute them together on your behalf.
...
The `requirements.yml` needs to be read - either on the command line by invocing `ansible-galaxy`, or automatically by Ansible Tower during project check outs. In both cases the file is read, and the roles are checked out and stored locally, and the roles can be called in playbooks. The advantage of Tower here is that it takes care of all that - including authorization to the Git repo, finding a proper place to store the role, updating it when needed and so on. Due to that reasons we will focus only on the Tower execution here.

In this example, we will include a role which ships a simple `index.html` file as template and reloads the apache web server. The role is shared in Gitea at link:http://control.example.com/gitea/git/shared-apache-role .
...
Next, we reference the role itself in our playbook:
...
Do not forget to add, commit and push the changes!
...
[master 3b87fda] Add roles/requirements.yml referencing shared role
...
And you are done. You successfully used a shared role provided from a central source and used it integrated into your automation code. This way, you can limit your automation code to things really relevant and individual to you and your environment, while everything shreable is consumed from a shared resource.
