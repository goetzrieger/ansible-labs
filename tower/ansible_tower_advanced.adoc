...
In this chapter we will show you how to set up the inventory step-by-step with *tower-cli* to have an example to better understand how it works. All other following steps setting up projects and so on will just be shown with the full *tower-cli* command and not explained step-by-step. Instead, we will write them all into a shell script to have an example how to boot strap a Tower from bottom up. This serves as an example how to automatically configure your Tower, for example for test cases.
...
Create a simple shell script on the control host:

----
[root@control ~]# cat setup-tower.sh
#!/bin/bash
...
Next, make the script executable, and launch it:
...
If you run the script a second time, you will see that *tower-cli* is idempotent.
...
As you can see the ssh key is read in via a sub-shell. Since JSON POST data need to be on one line, we replace all new lines in the ssh key file with a *\n*.

Add this line to *setup-tower.sh*.
...
Note the different *credential-type* in the command.

=== Create the Project

Now with the SCM credentials configured, we add a project to import the playbooks. Add the appropriate *tower-cli* line to it:
...
Note that the first parameter to *tower-cli* is different here since we work on the resource *project*.
