...
As mentioned one of the puproses of *tower-cli* is to use it to automatically configure more complex Tower setups. In such cases, multiple *tower-cli* commands are put togerther in a script. We follow that practice in our example, and create a shell script on the control host with all commands you have to run to bootstrap Tower.
