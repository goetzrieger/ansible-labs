...
generic code:: All content that will be used across multiple projects. In most cases this consists of [Ansible roles](https://docs.ansible.com/ansible/latest/user_guide/playbooks_reuse_roles.html). How a role itself is structured is [sufficiently described](https://galaxy.ansible.com/docs/contributing/creating_role.html) and will not be part of this workshop. Instead we will focus on how to structure when multiple roles are used in parallel. 

data:: This is mostly the inventory or the inventory scripts and the corresponding variables for hosts and groups. In many use cases it is advisable to have a dedicated inventory for each life-cycle environment. They can all be in the same Git repository, each in its own directory (e.g. dev, test, qa, prod). Alternatively, for example in larger environments or with dedicated teams per environment there can be one Git repository for each environment. How inventories are structured is [sufficiently described](https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html) and is not part of this workshop. We recommend to put special focus on [splitting out host and group data](https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html#splitting-out-host-and-group-specific-data).
...
So, let's focus on the repository where you have the playbooks you'll call from within your Tower job templates. We recommend to have a structure somewhat similar to the one of a role, the structure has been thought out for a reason and this makes it easier to modularize your code once you decide to use roles. All of our recommendations are aligned to the [Ansible best practices](https://docs.ansible.com/ansible/latest/user_guide/playbooks_best_practices.html#content-organization) and are explained in more detail there.
...
Let's now assume that we also have external roles needed by our playbooks, those can either come from [Ansible Galaxy](https://galaxy.ansible.com/) or, more commonly in an enterprise environment, directly from a (private) Git repository.
