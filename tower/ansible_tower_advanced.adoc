...
The `requirements.yml` needs to be read - either on the command line by invocing `ansible-galaxy`, or automatically by Ansible Tower during projct check outs. In both cases the file is read, and the roles are checked out and stored locally. We will focus on the Tower use case here.
