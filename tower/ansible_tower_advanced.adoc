...
Writing objects: 100% (32/32), 4.06 KiB | 0 bytes/s, done.
...
=== Optional: Adding external roles
...
----
[root@control-dff2 ~]# tower-cli user create --username jsmith --email jsmith@example.com --password redhat --verbose
...
----
[root@control-dff2 ~]# tower-cli user delete --username jsmith --verbose
...
WARNING: The "Getting the record" is (sadly) a bit misleading...  you need to add `?username=jsmith` to filter on the username:

Check if the user exists:

----
[root@control-dff2 ~]# curl -k -H 'Content-Type: application/json' --user admin:r3dh4t1! \
	-X GET https://tower1.example.com/api/v2/users/?username=jsmith
----
...
[root@control-dff2 ~]# curl -k -H 'Content-Type: application/json' --user admin:r3dh4t1! \
	...
	-X POST https://tower1.example.com/api/v2/users/?username=jsmith
----
...
* Take a job template and add a Survey to it with two to three fields of different types. It doesn't matter what you put into the survey. What about:
** *PROMPT:* What
** *ANSWER VARIABLE NAME:* what
** *ANSWER TYPE:* Text
** *+ADD*
** *PROMPT:* When
** *ANSWER VARIABLE NAME:* when
** *ANSWER TYPE:* Integer
** *DEFAULT ANSWER:* 0
** *+ADD*
** *SAVE*
* Note down the ID of the job template, you'll find it in the brower's URL.
* Go back to the API and because we're lazy, go directly to \https://tower2-<GUID>.rhpds.opentlc.com/api/v2/job_templates/*<ID>*/
* There we click on the `survey_spec` in the `related` section.
* Click the *GET* button to get the result in JSON format
* and finally "Save" or "Copy" the result into a file `job_survey.json` (we'll need it for the next step).
