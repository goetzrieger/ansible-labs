...
It's a common part of the learning curve for Ansible and Ansible Tower: At some point you have to really start structuring your content... Playbooks, Templates, Files and so on. Because you should have your content in version control anyway (most likely this will be Git these days) this will be about how to structure your Git repositories. What structure works best for you is of course highly dependend on your specific requirements, but some common ground rules apply.

The first recommendation is to separate _specific code_ from _reusable code_ from _data_:

specific code:: Playbooks and their direct dependencies you're not sharing or not planning to share outside the realm of your project or team. 

generic code:: Basically Ansible roles that will be used across multiple projects. How roles are structured is https://galaxy.ansible.com/docs/contributing/creating_role.html[sufficiently described] and should become second nature to you. When talking about Ansible Roles `ansible-galaxy` should come to your mind immediately. Currently to pull roles from `ansible-galaxy`, you will need one Git repository for each role (this might change with https://galaxy.ansible.com/docs/mazer/index.html[Mazer]).  We'll discuss here how to use such role repositories within Tower.

data:: this is mostly your inventory or inventories. You probably want one inventory for each life-cycle environment, you can have them all in the same Git repository, each in its own directory (e.g. dev, test, qa, prod) or have one Git repository for each environment, mainly in a balance between simplicity and access control. How inventories are structured is https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html[sufficiently described] and should be known (and the recommendation is to https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html#splitting-out-host-and-group-specific-data[split out host and group data] but for the most simple cases).

CAUTION: what you definitely do _not_ want is one disjunst code repository for each environment, as it would go against the purpose of testing the _same_ code as you push it through your life-cycle, only varying the data / inventory. If you have difficulties to keep the same code throughout all your environments, you probably need to re-think how you structure it vs. what you put in your inventories.

=== Main repository

So, let's focus on the repository where you have the playbooks you'll finally call from within your job templates. The simple recommendation is to have a structure similar to the one of a role, the structure has been thought out for a reason and this makes it easier to modularize your code once you decide to use roles.

FIXME:: how to create a Git repo in Gitea???

----
git clone http://control.example.com/gitea/git/structured-playbooks.git # FIXME: correct URL?
cd structured-playbooks
ansible-galaxy init --offline dummy
mv dummy/{README.md,files,tasks,templates,tests,vars} . # <1>
----
<1> do NOT forget the dot (*.*) at the end!!!

You can then check what remained in the dummy directory, and convince yourself that there is nothing relevant for playbooks.

----
rm -fr dummy
mkdir {plays,roles}
echo 'autocmd FileType yaml setlocal ai ts=2 sw=2 et' > files/vimrc
echo 'Ansible has your machine under control, {{ ansible_user | default(\'someone\') }} wishes you welcome!' > templates/motd.j2
for file in plays tasks tests vars; do touch ${file}/some${file}.yml; done
touch top-level_playbook.yml
touch roles/requirements.yml # <1>
ansible-galaxy init --offline --init-path=roles somerole
----
<1> we explain in the next chapter what this is for.

The directory `plays` is for playbooks you're not primarily planning to call directly but only via `import_playbook` in your top-level playbooks. Those top-level playbooks live in the root directory of your repository so that they are easy to find. A naming convention can help to understand the structure of your playbooks (e.g. something like `<nr>_<verb>_<object>.yml` as in `05_setup_webserver.yml`).

IMPORTANT: even if a good playbook is generally self-explanatory, the context and which one to use in which occasion generally isn't, so that it makes sense to have a top-level README-file (in whichever format) to explain this, as well as where to find the fitting inventory repositories.

Call now `find .` (FIXME: or is tree installed?) to get a feel how a complete structure can look like.
...
NOTE: you can also have `group_vars` and `host_vars` next to your playbook(s) so that the variables defined in those directories are implicitly read by Ansible. This is acceptable but not recommended as it generally amounts to break the separation of code from data.
