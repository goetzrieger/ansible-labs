...
=== So little time and so much to try...

WARNING: To be honest we got carried away slighty while trying to press all these cool features into a 2-hours lab session. We decided to flag the last two chapters as "optional" instead of taking them out. If you find the time to run them, cool! If not, the lab guide will stay where it is, feel free to go through these lab tasks later (you don't need a Tower cluster for this).  
...
* All labs where prepared using *Vim*, but we understand not everybody loves it. Feel free to use an alternative editor, in the lab environment *Midnight Commander* (just run *mc*, function keys can be reached via Esc-<n> or clicked with the mouse) and *Nano* (run *nano*) are installed on the control host.
