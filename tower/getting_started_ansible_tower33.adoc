...
To start using Ansible Tower, some concepts and naming conventions should be 
known.
...
* Go to *Resources -> Inventories -> Example Inventory*, switch to the *HOSTS* 
view and edit `host1.example.com` by clicking the pen icon.
...
* Copy the `Apache` template using the copy icon in the *Templates* view
* Change the name to `Apache Ask` 
...
TIP: Note the two variables marked by `{{...}}` used in the template!
...
* Go to the *Projects* view and start an SCM update for "Control Git Repo" (the 
arrows button). 
...
** Defines two variables: `dev_content: default dev content` and `prod_content: default prod content` in the *EXTRA VARIABLES FILED*
...
----
tower-cli inventory create -n inventory-cli --organization 
Default --force-on-exists
----

Add a host to the inventory:
----
(ansible)[root@tower ~]# tower-cli host create -n host1.example.com 
--inventory inventory-cli --force-on-exists
----

Create a new Project:
----
(ansible)[root@tower ~]# tower-cli project create -n project-cli -d "test 
project" --organization Default --scm-type git --scm-credential "Control 
Git" --scm-url "control.example.com:/home/git/git-webdev"
----
