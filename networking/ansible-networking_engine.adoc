...
Now the configuration backup should have worked on both devices. The backup is written to the directory `backup`. Have a look (your filenames will differ, of course):
...
----
[ansible@control ansible-files]$ ansible csr -m ios_banner -a 'banner=login text="Managed by Ansible!" state=present provider={{ cisco_connect }} authorize=yes auth_pass=cisco'
----
----
[ansible@control ansible-files]$ ansible csr -m ios_banner -a 'banner=login text="Managed by Ansible!" state=absent provider={{ cisco_connect }} authorize=yes auth_pass=cisco'
----
