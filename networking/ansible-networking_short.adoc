...
Verify that the Cisco´s accept SSH connections with user/password authentication from the control node:
----
...
* When configuring an _network device_ Ansible will call the module _locally_, the module then handles the connection to the device. SSH in this lab, but it may be some API or CLI call, too. This happens mostly transparent to the user. 
...
TIP: These settings can be configured in a number of ways and places, e.g as environment variables. Using the group variables is a good way to stay sane... ;-)
...
If you get into issues like this, it's best to log into the devices and in the shell execute what the module would do:
...
We have to provide a password to enter privileged mode. For practice first look up the documentation for the module and look for `authorize` and `auth_pass`. Now we just have to tell the module to use authorization:

----
[ansible@control ansible-files]$ ansible csr -m ios_config -a 'backup=yes provider={{cisco_connect}} authorize=yes auth_pass=cisco' 
----

Now the configuration backup should have worked on both devices. Have a look at the backup files in the `backup/` folder.
...
* Log out of host1.example.com and run the Playbook a second time to get used to the change in the output.
