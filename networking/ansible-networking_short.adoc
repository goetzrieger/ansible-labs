...
TIP: You could easily use this to do regular backups of your device configurations. One thing to keep in mind though: Ansible deletes older files for the same host even while they are timestamped. 
...
== Network Automation using Playbooks
...
Let's write the Playbook. Create the file `~/ansible-files/check_config.yml` and add this content:
